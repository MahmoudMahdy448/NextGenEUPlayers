WITH base AS (SELECT * FROM {{ ref('stg_fbref__standard') }}), -- We still need Standard for Age/Nation/Minutes
     gk_basic AS (SELECT * FROM {{ ref('stg_fbref__goalkeeping') }}),
     gk_adv   AS (SELECT * FROM {{ ref('stg_fbref__adv_goalkeeping') }})
